...
export default function AnalyticsPage() {
  ...
  const { data: stats, isLoading: statsLoading } =
    useTransactionStats(transactionFilters);
      ...
      <div className="space-y-6">
            ...
            <div className="flex rounded-lg border overflow-hidden text-xs">
              {(["bar", "area", "line"] as const).map((t) => (
                <button
                  ...
                  className={`px-3 py-1.5 capitalize transition-colors ${trendType === t ? "bg-primary text-white" : "hover:bg-muted"}`}
                >
                  {t}
                </button>
              ))}
        ...
        <div className="rounded-xl border bg-card p-5">
          <h3 className="font-semibold mb-4">
            Daily Spending Heatmap — Last 3 Months
          </h3>
