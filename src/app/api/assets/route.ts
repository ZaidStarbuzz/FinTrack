...
export async function PATCH(req: Request) {
  try {
    ...
    const { data, error } = await supabase.from('assets').update(body).match({ id, user_id: payload.sub }).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ asset: data })
  } catch (e:any) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const id = body.id
    if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 })
    const { error } = await supabase.from('assets').delete().match({ id, user_id: payload.sub })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function GET(req: Request) {
  try {
    ...
    const payload: any = verifyToken(token)
    if (!payload) return NextResponse.json({ error: 'Invalid token' }, { status: 401 })
    const { data, error } = await supabase.from('assets').select('*').eq('user_id', payload.sub)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
