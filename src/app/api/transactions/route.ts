...
export async function PATCH(req: Request) {
  try {
    ...
    const { data, error } = await supabase.from('transactions').update(body).match({ id, user_id: payload.sub }).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ transaction: data })
  } catch (e: any) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const { error } = await supabase.from('transactions').delete().match({ id, user_id: payload.sub })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
