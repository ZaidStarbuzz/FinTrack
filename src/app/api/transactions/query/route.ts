  ...
  try {
    ...
    if (filters.categoryIds && Array.isArray(filters.categoryIds))
      ...
      const { data: nameMatches, error: nameErr } = await supabase
        .from("categories")
        .select("id")
        .ilike("name", `%${filters.categoryName}%`)
        .or(`user_id.eq.${payload.sub},user_id.is.null`);
      if (nameErr)
    ...
    if (filters.includeCategoryChildren && categoryIds && categoryIds.length) {
      ...
      const childrenMap: Record<string, string[]> = {} as Record<
        string,
        string[]
      >;
    ...
    let query = supabase
      .from("transactions")
      .select(
        `*, account:accounts!transactions_account_id_fkey(*), transfer_account:accounts!transactions_transfer_account_id_fkey(*), category:categories(*)`,
        { count: "exact" },
      )
