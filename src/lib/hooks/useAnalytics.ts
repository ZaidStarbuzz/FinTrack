    ...
    queryFn: async () => {
      ...
      const [currRes, prevRes, accountsRes] = await Promise.all([
        fetch("/api/transactions/stats", {
          ...
          credentials: "include",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ filters: { from: currStart, to: currEnd } }),
        }),
        fetch("/api/transactions/stats", {
          method: "POST",
          credentials: "include",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ filters: { from: prevStart, to: prevEnd } }),
        }),
      ...
      if (!currRes.ok)
        throw new Error(
          currPayload?.error || "Failed to fetch current month stats",
        );
      if (!prevRes.ok)
        throw new Error(
          prevPayload?.error || "Failed to fetch previous month stats",
        );
      if (!accountsRes.ok)
      ...
      const totalBalance = (accountsPayload.accounts || []).reduce(
      ...
      const burnRate = calculateBurnRate([
        prevPayload.expenses || 0,
        currPayload.expenses || 0,
      ]);
      const runway = calculateRunway(totalBalance, burnRate);

      const insights: any[] = [];
      const savingsRate =
        (currPayload.income || 0) > 0
          ? (((currPayload.income || 0) - (currPayload.expenses || 0)) /
              (currPayload.income || 0)) *
            100
          : 0;
      ...
      if (savingsRate >= 20)
        ...
        insights.push({
          ...
          message: `You're saving only ${savingsRate.toFixed(1)}%. Try to save at least 20%.`,
        });

      if ((prevPayload.expenses || 0) > 0) {
        const diff =
          (((currPayload.expenses || 0) - (prevPayload.expenses || 0)) /
            (prevPayload.expenses || 0)) *
          100;
        if (diff > 20)
          insights.push({
            ...
            message: `Expenses increased by ${diff.toFixed(1)}% compared to last month.`,
          });
      }
      ...
      if (topCategories[0])
        insights.push({
          id: "3",
          type: "info" as const,
          title: `Top Spend: ${topCategories[0].category_name}`,
          message: `₹${(topCategories[0].total_amount || 0).toLocaleString("en-IN")} spent this month.`,
        });
      ...
      if (runway < 3 && runway > 0)
        insights.push({
          ...
          type: "warning" as const,
          title: "Low Runway!",
          message: `At current spending, your funds last ${runway} months.`,
        });
      else if (runway >= 6)
        insights.push({
          id: "4",
          ...
          message: `Your funds can last ${runway}+ months at current spending.`,
        });
