...
export function useImportCSV() {
  ...
  return useMutation({
    mutationFn: async ({ rows, accountId }: { rows: CSVRow[]; accountId: string }) => {
      ...
      const transactions = rows.map(row => {
        const hash = btoa(`${row.date}-${row.description}-${row.amount}`).slice(0, 32)
        const categoryId = row.category ? catMap.get(row.category.toLowerCase()) : null
        const amount = Math.abs(parseFloat(row.amount.replace(/[^0-9.-]/g, '')))
        const type = row.type?.toLowerCase() === 'income' ? 'income'
          : row.type?.toLowerCase() === 'transfer' ? 'transfer'
          : parseFloat(row.amount) < 0 ? 'expense' : (row.type === 'income' ? 'income' : 'expense')
        return {
          user_id: user.id,
      ...
      const { data, error } = await supabase
        .from('transactions')
        .upsert(transactions, { onConflict: 'import_hash', ignoreDuplicates: true })
        .select()
...
export function useExportCSV() {
  return useMutation({
    mutationFn: async (filters: { from?: string; to?: string }) => {
      ...
      const headers = ['Date', 'Description', 'Amount', 'Type', 'Account', 'Category', 'Tags', 'Notes', 'Status']
